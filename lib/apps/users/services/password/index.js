...
Password.prototype.reset = function(userId, resetPassword) {
...
Password.prototype.resetByEmail = function(email, resetPassword) {
  var that = this;
  
  return User.listByEmail(email).then(function(users) {
    if (email == null || users.length === 0) return;

    return that.reset(users[0].id, resetPassword);
  });
};
