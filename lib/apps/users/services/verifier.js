...
var email = function(user, host) {
  ...
  return publish('email', { user: user, template: 'user.password', data: user, force: true }, { host: host });
};
...
Verifier.prototype.checkEmailAndVerification = function(id, user) {
  ...
  return User.get(id).then(function(existing) {
    ...
    if (existing.email === user.email || user.email == null) return user;

    return checkForExistingUsers(user.email).then(function() {
