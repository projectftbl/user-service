...
var list = Promise.method(function(query) {
  ...
  if (query.accesstoken) return User.listByAccessToken(query.accesstoken, query);
  if (query.q) return User.search(query.q, query);

  return [];
});

var populate = Promise.method(function(user) {
  if (user == null || user.roles == null) return user;

  return Role.listByIdsOrHandles(user.roles).then(function(roles) {
...
Finder.prototype.search = Promise.method(function(query, options) {
  if (query == null || query === '') return [];
  return User.search(query, options);
});

Finder.prototype.total = function(query, options) {
  return User.total(query, options);
};
